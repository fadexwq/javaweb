...
<script type="text/javascript">
    ...
    function menuFormatter(value,row,index){
        if ("menu" == value) {
            return "<span style='color:orangered'>菜单角色</span>";
        }
        return "<span style='color:deepskyblue'>普通角色</span>"
    }
    /*
    *
    * value :当前行 field的值
    ...
    */
    function operationFormatter(value,row,index){
        ...
        var html = "<span onclick='role_edit("+value+")' style='cursor: pointer;color:blue' class='glyphicon glyphicon-edit'></span>&nbsp;&nbsp;&nbsp;";

        html += "<span onclick='role_del("+value+")' style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span>"
...
<script type="text/javascript">
    ...
    /*角色-增加*/
    function role_add(){
        layer_show("添加管理","role/add.do",800,500);
    }

    /*角色-编辑*/
    function role_edit(roleId){
        layer_show("添加管理","role/add.do?roleId="+roleId,800,500);
    }

    /*角色-删除*/
    function role_del(id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("role/delete.do",{roleId:id},function(data){
                //弹出一个提示消息
